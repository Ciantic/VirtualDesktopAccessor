...
impl Desktop {
    ...
    pub fn get_id(&self) -> Result<GUID> {
        com_objects().as_result()?.get_desktop_id(&self.0)
    }

    pub fn get_index(&self) -> Result<u32> {
        com_objects().as_result()?.get_desktop_index(&self.0)
    }

    /// Get desktop name
    pub fn get_name(&self) -> Result<String> {
        com_objects().as_result()?.get_desktop_name(&self.0)
    }

    /// Set desktop name
    pub fn set_name(&self, name: &str) -> Result<()> {
        com_objects().as_result()?.set_desktop_name(&self.0, name)
    }

    /// Get desktop wallpaper path
    pub fn get_wallpaper(&self) -> Result<String> {
        com_objects().as_result()?.get_desktop_wallpaper(&self.0)
    }
    ...
    pub fn set_wallpaper(&self, path: &str) -> Result<()> {
        com_objects()
            .as_result()?
            .set_desktop_wallpaper(&self.0, path)
    }
}
    ...
    T: Into<Desktop> + Copy + Clone + Send + 'static,
{
    com_objects()
        .as_result()?
        .switch_desktop(&desktop.into().into())
}
    ...
    F: Into<Desktop>,
{
    com_objects()
        .as_result()?
        .remove_desktop(&desktop.into().into(), &fallback_desktop.into().into())
}
    ...
    T: Into<Desktop>,
{
    com_objects()
        .as_result()?
        .is_window_on_desktop(&hwnd, &desktop.into().into())
}
    ...
    T: Into<Desktop>,
{
    Ok(com_objects()
        .as_result()?
        .move_window_to_desktop(hwnd, &desktop.into().into())?)
}

/// Create desktop
pub fn create_desktop() -> Result<Desktop> {
    Ok(com_objects().as_result()?.create_desktop()?.into())
}

/// Get current desktop
pub fn get_current_desktop() -> Result<Desktop> {
    Ok(com_objects().as_result()?.get_current_desktop()?.into())
}
...
pub fn get_desktop_by_window(hwnd: HWND) -> Result<Desktop> {
    com_objects()
        .as_result()?
        .get_desktop_by_window(&hwnd)
        .map(Desktop)
}

/// Get desktop count
pub fn get_desktop_count() -> Result<u32> {
    com_objects().as_result()?.get_desktop_count()
}
...
pub fn is_pinned_window(hwnd: HWND) -> Result<bool> {
    com_objects().as_result()?.is_pinned_window(&hwnd)
}

/// Pin window
pub fn pin_window(hwnd: HWND) -> Result<()> {
    com_objects().as_result()?.pin_window(&hwnd)
}

/// Unpin window
pub fn unpin_window(hwnd: HWND) -> Result<()> {
    com_objects().as_result()?.unpin_window(&hwnd)
}

/// Is pinned app
pub fn is_pinned_app(hwnd: HWND) -> Result<bool> {
    com_objects().as_result()?.is_pinned_app(&hwnd)
}

/// Pin app
pub fn pin_app(hwnd: HWND) -> Result<()> {
    com_objects().as_result()?.pin_app(&hwnd)
}

/// Unpin app
pub fn unpin_app(hwnd: HWND) -> Result<()> {
    com_objects().as_result()?.unpin_app(&hwnd)
}
